...
class LanguageIDModel(Module):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        self.hidden = Linear(47, 128)
        self.hidden_transform = Linear(128, 128)
    ...
    def run(self, xs):
        ...
        for x_t in xs:
            z=self.hidden(x_t) + self.hidden_transform(h)
            h = relu(z)
...
class Attention(Module):
    ...
    def forward(self, input):
        ...
        Q_transposed = torch.movedim(Q, 1, 2)  
        attention_scores = torch.matmul(K, Q_transposed)  
        scaled_attention_scores = attention_scores / (self.layer_size ** 0.5)
        
        masked_attention_scores = scaled_attention_scores.masked_fill(self.mask[:,:,:T,:T] == 0, float('-inf'))
        
        attention_weights = softmax(masked_attention_scores, dim=-1)
